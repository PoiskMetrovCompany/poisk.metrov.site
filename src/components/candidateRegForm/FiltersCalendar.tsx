...
const setMockVacancies = () => {
  const mockVacancies = [
    {value: 'showAll', text: 'Показать все', title: null},
    {value: '1', text: 'Frontend разработчик', title: 'Frontend разработчик'},
  ...
  const handleDateClick = (dateStr: string, year?: number, month?: number, day?: number) => {
  ...
  const generateCalendar = (year: number, month: number): (DateCell | null)[][] => {
  ...
  // Функции для проверки выбранности дат
    ...
    return startDate.getTime() === date.getTime() || Boolean(endDate && endDate.getTime() === date.getTime());
    };
  ...
  const isEndDate = (dateStr: string) => {
    ...
    return Boolean((startDate && startDate.getTime() === monthDate.getTime()) ||
  ...
  const isMonthEndDate = (year: number, monthIndex: number) => {
    ...
    return Boolean((startDate && startDate.getTime() === yearDate.getTime()) ||
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (isCustomSelectOpen && !(e.target as HTMLElement).closest('.custom-select')) {
        setIsCustomSelectOpen(false);
      }
      if (isOpen && calendarPanelRef.current && !calendarPanelRef.current.contains(e.target as Node) && filtersButtonRef.current && !filtersButtonRef.current.contains(e.target as Node)) {
        onClose();
      }
    };
