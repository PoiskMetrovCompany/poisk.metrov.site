...
const CandidatesTable: React.FC<CandidatesTableProps> = ({
  ...
  const [singleDownloadLoading, setSingleDownloadLoading] = useState<Record<string, boolean>>({});
  ...
  const handleSingleDownload = async (vacancyKey: string, candidateName: string) => {
  ...
  useEffect(() => {
    const handleClickOutside = (e: MouseEvent) => {
      if (isFormatDropdownOpen && !(e.target as HTMLElement).closest('.download-button-group')) {
        setIsFormatDropdownOpen(false);
      }
    };
    ...
    <section style={{flexWrap: 'wrap', minHeight: 'auto'}}>
      ...
      {candidates.length === 0 ? (
        ...
        <>
          ...
          <FormRow className="w-80" justifyContent="space-between" style={{marginTop: '2rem'}}>
