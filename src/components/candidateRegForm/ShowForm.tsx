...
const ShowForm: React.FC<ShowFormProps> = ({ vacancyKey, setSelectedVacancyKey, selectedCity }) => {
  ...
  useEffect(() => {
        const applyStyles = (element: Element) => {
            ...
            const mainElement = element.tagName === 'MAIN' ? element as HTMLElement : element.querySelector('main');
            if (mainElement) {
            ...
            const sectionElement = element.tagName === 'SECTION' ? element as HTMLElement : element.querySelector('section');
            if (sectionElement) {
                ...
                mutations.forEach((mutation) => {
                    mutation.addedNodes.forEach((node) => {
                        if (node.nodeType === Node.ELEMENT_NODE) {
                            ...
                            // Проверяем section
                            if (element.tagName === 'SECTION' || element.querySelector('section')) {
                                applyStyles(element);
                            }
                        }
                    });
          ...
          <div className="center-card big">
            ...
            {(candidateData.level_educational || educationalInstitutions.length > 0 || courses.length > 0) && (
              <>
                ...
                {educationalInstitutions.length > 0 && educationalInstitutions.map((institution: any, index: number) => (
