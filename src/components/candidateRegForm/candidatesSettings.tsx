...
const CandidatesSettings: FC = () => {
  ...
  const saveNewRole = async () => {
    if (newRole.trim()) {
      ...
      try {
        ...
        console.error('Тип ошибки при создании:', (error as Error).constructor.name);
        console.error('Сообщение ошибки при создании:', (error as Error).message);
        console.error('Stack trace при создании:', (error as Error).stack);

        if ((error as any).response) {
          console.error('Ответ сервера при ошибке создания:', (error as any).response);
          console.error('Статус ошибки создания:', (error as any).response.status);
  ...
  const confirmDelete = async () => {
    ...
    try {
      ...
      console.error('Тип ошибки при удалении:', (error as Error).constructor.name);
      ...
      if ((error as any).response) {
        console.error('Ответ сервера при ошибке удаления:', (error as any).response);
    ...
    <>
      <main>
        <section style={{minHeight: '0', flexWrap: 'wrap'}}>
          ...
          <div className="center-card big w-70">
